...
func (l *Logger) output(s severity, depth int, txt string) {
	...
	switch s {
	case sInfo:
		_ = l.infoLog.Output(3+depth, txt)
	case sWarning:
		_ = l.warningLog.Output(3+depth, txt)
	case sError:
		_ = l.errorLog.Output(3+depth, txt)
	case sFatal:
		_ = l.fatalLog.Output(3+depth, txt)
	default:
...
func (l *Logger) Close() {
	...
	for _, c := range l.closers {
		if err := c.Close(); err != nil {
			_, _ = fmt.Fprintf(os.Stderr, "Failed to close log %v: %v\n", c, err)
		}
	}
}
